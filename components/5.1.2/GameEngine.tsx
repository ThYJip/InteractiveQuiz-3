...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#F0F9FF]">
      {/* Back Button (Sky Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-sky-50 border-2 border-sky-200 hover:border-sky-300 rounded-2xl text-sky-800/80 hover:text-sky-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-sky-900/60">CHAPTER 5.1.2</h2>
          <span className="font-mono text-sky-800/60 tracking-widest">NAV ARGUMENTS</span>
      </div>
    </div>
  );
};
