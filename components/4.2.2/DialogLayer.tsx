...
const DialogLayer: React.FC<Props> = ({ speaker, text, onNext, canProceed, isLastStep, onHome }) => {
    ...
    switch (speaker) {
      ...
      case 'Chi':
        return { 
            ...
            border: 'border-green-400', 
            text: 'text-green-600', 
            icon: <Cat size={20} className="text-green-500" /> 
        };
      default:
  ...
  return (
    <div className="fixed bottom-6 left-4 right-4 z-50 flex justify-center pointer-events-none">
      <div className="relative w-full max-w-4xl bg-white rounded-[2rem] p-8 pb-10 shadow-[0_20px_50px_-12px_rgba(34,197,94,0.4)] border border-green-100 pointer-events-auto">
