...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#F1F8E9]">
      {/* Back Button (Green Forest Style) */}
      {onBack && (
        <button 
          onClick={onBack}
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-green-900/60">CHAPTER 2.3</h2>
          <span className="font-mono text-green-800/60 tracking-widest">REMEMBER ME</span>
      </div>
    </div>
  );
};
