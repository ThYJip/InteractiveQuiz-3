...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#F3E8FF]">
      {/* Back Button (Purple Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-purple-50 border-2 border-purple-200 hover:border-purple-300 rounded-2xl text-purple-800/80 hover:text-purple-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-purple-900/60">CHAPTER 3.2.1</h2>
          <span className="font-mono text-purple-800/60 tracking-widest">SLOT PATTERN</span>
      </div>
    </div>
  );
};
