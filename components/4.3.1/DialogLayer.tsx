...
const DialogLayer: React.FC<Props> = ({ speaker, text, onNext, canProceed, isLastStep, onHome }) => {
    ...
    switch (speaker) {
      ...
      case 'Chi':
        return { 
            ...
            icon: <Cat size={20} className="text-orange-500" /> 
        };
      default:
  ...
  return (
    <div className="fixed bottom-6 left-4 right-4 z-50 flex justify-center pointer-events-none">
      <div className="relative w-full max-w-4xl bg-white rounded-[2rem] p-8 pb-10 shadow-[0_20px_50px_-12px_rgba(251,146,60,0.4)] border border-orange-100 pointer-events-auto">
        ...
        {!isTyping && (
            <>
                ...
                {!isLastStep && canProceed && (
                    <button 
                        onClick={onNext}
                        className="absolute bottom-6 right-8 text-orange-500 animate-bounce flex items-center gap-1 hover:text-orange-700 transition-colors"
                    >
                ...
                {isLastStep && onHome && (
                     <button 
                        onClick={onHome}
                        className="absolute bottom-6 right-8 bg-orange-100 hover:bg-orange-200 text-orange-700 px-5 py-2 rounded-xl font-bold flex items-center gap-2 shadow-sm transition-all animate-pulse"
                    >
                        <Home size={18} />
                        <span>返回大厅</span>
                    </button>
