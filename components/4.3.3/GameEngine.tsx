...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#FEF2F2]">
      {/* Back Button (Red Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-red-50 border-2 border-red-200 hover:border-red-300 rounded-2xl text-red-800/80 hover:text-red-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-red-900/60">CHAPTER 4.3.3</h2>
          <span className="font-mono text-red-800/60 tracking-widest">EVENT HOISTING</span>
      </div>
    </div>
  );
};
