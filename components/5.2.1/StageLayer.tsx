...
import InteractiveAnimLab from './InteractiveAnimLab';
import { Image as ImageIcon, Move, Zap, Activity } from 'lucide-react';
...
const StageLayer: React.FC<Props> = ({ step, onInteractiveComplete }) => {
  ...
  const renderContent = () => {
    switch (viewType) {
      case 'IMAGE':
        return (
          <div className="flex flex-col items-center justify-center h-full text-fuchsia-900 overflow-y-auto p-4">
            <div className="w-80 h-64 bg-fuchsia-50 rounded-[3rem] flex items-center justify-center mb-6 border-4 border-dashed border-fuchsia-400 shadow-inner relative overflow-hidden group shrink-0">
                <div className="absolute inset-0 bg-gradient-to-br from-fuchsia-100/50 to-purple-100/50"></div>
                <Move size={80} className="text-fuchsia-700/20" />
                <div className="absolute bottom-4 right-4 bg-white/80 px-3 py-1 rounded-full text-xs font-mono text-fuchsia-800 flex items-center gap-1">
                    AI_SCENE_GEN
                </div>
            </div>
            <p className="text-xl font-bold text-fuchsia-900/80 text-center max-w-lg leading-relaxed bg-white/60 px-6 py-4 rounded-2xl backdrop-blur-sm shadow-sm border border-fuchsia-200/50">
          ...
          <div className="w-full max-w-3xl bg-[#1e293b] rounded-2xl shadow-2xl overflow-hidden border-4 border-slate-800 flex flex-col max-h-full transform -rotate-1 hover:rotate-0 transition-transform duration-500">
            <div className="bg-slate-900 px-4 py-3 flex items-center justify-between border-b border-slate-700 shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400 ml-3 font-mono font-bold tracking-wider">Animation.kt</span>
                </div>
            </div>
            <div className="overflow-auto p-6 custom-scrollbar bg-[#0f172a]">
                <pre className="text-sm md:text-base font-mono leading-loose text-fuchsia-100 whitespace-pre-wrap">
      ...
      case 'TECH_SUMMARY':
         return (
             <div className="w-full max-w-4xl h-full overflow-y-auto custom-scrollbar p-4 flex flex-col items-center">
                 <div className="text-center flex flex-col items-center p-6 bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border-4 border-fuchsia-100 animate-in zoom-in duration-500 w-full mb-24">
                     <Zap size={60} className="text-fuchsia-600 mb-4" />
                     <h1 className="text-2xl md:text-3xl font-black text-fuchsia-800 mb-6 tracking-tight">露营手册：简单动画</h1>
  ...
  return (
    <div className="w-full flex items-center justify-center p-4 relative z-0" style={{ height: 'calc(100vh - 240px)' }}>
      {/* Background Decor */}
      <div className="absolute bottom-0 w-full h-32 bg-gradient-to-t from-fuchsia-200/40 to-transparent -z-10 pointer-events-none"></div>
