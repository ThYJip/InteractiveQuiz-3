...
const DialogLayer: React.FC<Props> = ({ speaker, text, onNext, canProceed, isLastStep, onHome }) => {
    ...
    switch (speaker) {
      ...
      case 'Chi':
        return { 
            ...
            icon: <Cat size={20} className="text-fuchsia-500" /> 
        };
      default:
  ...
  return (
    <div className="fixed bottom-6 left-4 right-4 z-50 flex justify-center pointer-events-none">
      <div className="relative w-full max-w-4xl bg-white rounded-[2rem] p-8 pb-10 shadow-[0_20px_50px_-12px_rgba(217,70,239,0.4)] border border-fuchsia-100 pointer-events-auto">
        ...
        {/* Corner Brackets (Fuchsia Style) */}
        <div className="absolute top-4 left-4 w-6 h-6 border-t-4 border-l-4 border-fuchsia-300 rounded-tl-xl opacity-60" />
        <div className="absolute top-4 right-4 w-6 h-6 border-t-4 border-r-4 border-fuchsia-300 rounded-tr-xl opacity-60" />
        <div className="absolute bottom-4 left-4 w-6 h-6 border-b-4 border-l-4 border-fuchsia-300 rounded-bl-xl opacity-60" />
        <div className="absolute bottom-4 right-4 w-6 h-6 border-b-4 border-r-4 border-fuchsia-300 rounded-br-xl opacity-60" />

        {/* Content */}
        <div className="mt-2 pr-24">
            <p className="text-slate-700 text-lg md:text-xl leading-relaxed font-bold tracking-wide">
            {displayedText}
            {isTyping && <span className="inline-block w-2 h-5 bg-fuchsia-400 ml-1 animate-pulse align-middle rounded-full"></span>}
            </p>
        </div>
        ...
        {!isTyping && (
            <>
                ...
                {isLastStep && onHome && (
                     <button 
                        onClick={onHome}
                        className="absolute bottom-6 right-8 bg-fuchsia-100 hover:bg-fuchsia-200 text-fuchsia-700 px-5 py-2 rounded-xl font-bold flex items-center gap-2 shadow-sm transition-all animate-pulse"
                    >
                        <Home size={18} />
                        <span>返回大厅</span>
                    </button>
