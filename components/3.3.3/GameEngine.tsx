...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#ECFEFF]">
      {/* Back Button (Cyan Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-cyan-50 border-2 border-cyan-200 hover:border-cyan-300 rounded-2xl text-cyan-800/80 hover:text-cyan-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-cyan-900/60">CHAPTER 3.3.3</h2>
          <span className="font-mono text-cyan-800/60 tracking-widest">IMAGES & ICONS</span>
      </div>
    </div>
  );
};
