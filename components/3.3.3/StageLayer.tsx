...
import InteractiveImageLab from './InteractiveImageLab';
import { Image as ImageIcon, Eye, Brush, Maximize, AlertCircle } from 'lucide-react';
...
const StageLayer: React.FC<Props> = ({ step, onInteractiveComplete }) => {
      ...
      case 'IMAGE':
        return (
          <div className="flex flex-col items-center justify-center h-full text-cyan-900 overflow-y-auto p-4">
            <div className="w-80 h-64 bg-cyan-50 rounded-[3rem] flex items-center justify-center mb-6 border-4 border-dashed border-cyan-600/20 shadow-inner relative overflow-hidden group shrink-0">
                <div className="absolute inset-0 bg-gradient-to-br from-cyan-100/50 to-blue-100/50"></div>
                <ImageIcon size={80} className="text-cyan-700/20" />
                <div className="absolute bottom-4 right-4 bg-white/80 px-3 py-1 rounded-full text-xs font-mono text-cyan-800 flex items-center gap-1">
                    AI_SCENE_GEN
                </div>
            </div>
          ...
          <div className="w-full max-w-3xl bg-[#1e293b] rounded-2xl shadow-2xl overflow-hidden border-4 border-slate-800 flex flex-col max-h-full transform -rotate-1 hover:rotate-0 transition-transform duration-500">
            <div className="bg-slate-900 px-4 py-3 flex items-center justify-between border-b border-slate-700 shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400 ml-3 font-mono font-bold tracking-wider">Visuals.kt</span>
                </div>
            </div>
      ...
      case 'TECH_SUMMARY':
         return (
             <div className="w-full max-w-4xl h-full overflow-y-auto custom-scrollbar p-4 flex flex-col items-center">
                 <div className="text-center flex flex-col items-center p-6 bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border-4 border-cyan-100 animate-in zoom-in duration-500 w-full mb-24">
                     <ImageIcon size={60} className="text-cyan-600 mb-4" />
                     <h1 className="text-2xl md:text-3xl font-black text-cyan-800 mb-6 tracking-tight">露营手册：图片与图标</h1>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full text-left">
                         ...
                         <div className="bg-cyan-50 p-5 rounded-2xl border border-cyan-100 shadow-sm hover:shadow-md transition-shadow">
                             ...
                             <p className="text-sm text-slate-600 leading-relaxed">
                                 控制图片如何适应容器。<code>Crop</code> 用于填充背景，<code>Fit</code> 用于展示完整内容。避免默认的拉伸变形。
                             </p>
                         </div>
                         ...
                         <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100 shadow-sm hover:shadow-md transition-shadow">
                             ...
                             <p className="text-sm text-slate-600 leading-relaxed">
                                 Icon 是矢量符号，它自动读取 <code>LocalContentColor</code> 进行着色。这使得图标能自动适应按钮或背景的颜色变化。
                             </p>
                         </div>
                         ...
                         <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 col-span-1 md:col-span-2 shadow-sm hover:shadow-md transition-shadow">
                             ...
                             <p className="text-sm text-slate-600 leading-relaxed">
                                 <code>contentDescription</code> 是盲人用户的眼睛。信息类图片必须写描述，装饰类图片必须填 <code>null</code>。
                             </p>
                         </div>
                     </div>
                 </div>
         ...
         return (
             <div className="w-full h-full flex flex-col items-center justify-center p-4">
                 <div className="text-center animate-bounce flex flex-col items-center p-8 bg-white/60 backdrop-blur-md rounded-[3rem] shadow-2xl border-4 border-white mb-6">
                     <ImageIcon size={120} className="text-cyan-600 mb-6 drop-shadow-xl" />
                     <h1 className="text-5xl font-black text-cyan-800 mb-4 tracking-tight">Visual Master!</h1>
                     <p className="text-xl text-cyan-700 font-bold">
                        你构建了既美丽又包容的 UI！<br/>视觉体验满分！
                     </p>
                 </div>
             </div>
         )
  ...
  return (
    <div className="w-full flex items-center justify-center p-4 relative z-0" style={{ height: 'calc(100vh - 240px)' }}>
      {/* Background Decor */}
      <div className="absolute bottom-0 w-full h-32 bg-gradient-to-t from-cyan-200/40 to-transparent -z-10 pointer-events-none"></div>
