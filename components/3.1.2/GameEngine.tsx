...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#EEF2FF]">
      {/* Back Button (Indigo Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-indigo-50 border-2 border-indigo-200 hover:border-indigo-300 rounded-2xl text-indigo-800/80 hover:text-indigo-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-indigo-900/60">CHAPTER 3.1.2</h2>
          <span className="font-mono text-indigo-800/60 tracking-widest">HETEROGENEOUS LISTS</span>
      </div>
    </div>
  );
};
