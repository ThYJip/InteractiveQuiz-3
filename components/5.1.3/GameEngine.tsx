...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#FEFCE8]">
      {/* Back Button (Yellow Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-yellow-50 border-2 border-yellow-200 hover:border-yellow-300 rounded-2xl text-yellow-800/80 hover:text-yellow-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-yellow-900/60">CHAPTER 5.1.3</h2>
          <span className="font-mono text-yellow-800/60 tracking-widest">NAV OPTIONS</span>
      </div>
    </div>
  );
};
