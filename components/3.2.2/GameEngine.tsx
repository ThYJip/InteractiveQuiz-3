...
const GameEngine: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-[#EFF6FF]">
      {/* Back Button (Blue Style) */}
      {onBack && (
        <button 
          onClick={onBack}
          className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/90 hover:bg-blue-50 border-2 border-blue-200 hover:border-blue-300 rounded-2xl text-blue-800/80 hover:text-blue-700 transition-all shadow-sm hover:shadow-md group"
        >
      ...
      <div className="absolute top-8 right-8 text-right opacity-20 pointer-events-none select-none">
          <h2 className="text-4xl font-black text-blue-900/60">CHAPTER 3.2.2</h2>
          <span className="font-mono text-blue-800/60 tracking-widest">SCAFFOLDING</span>
      </div>
    </div>
  );
};
